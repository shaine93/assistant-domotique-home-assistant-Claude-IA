...
def _wizard_handle_message(texte):
    ...
    if step == "sms_free_user":
        CFG["free_mobile_user"] = texte.strip()
    ...
    if step == "sms_free_pass":
        CFG["free_mobile_pass"] = texte.strip()
        # Test d'envoi
        CFG["sms_method"] = "free_mobile"
        _wizard_save_config()
        code_test = "TEST"
        try:
            url = f"https://smsapi.free-mobile.fr/sendmsg?user={CFG['free_mobile_user']}&pass={CFG['free_mobile_pass']}&msg=AssistantIA:code_test_{code_test}"
    ...
    if step == "sms_email_addr":
        ...
        if "@" not in email:
            ...
            return True
        CFG["mail_dest"] = email
        CFG["smtp_user"] = email  # Par défaut = même email
...
def _wizard_complete():
    ...
    log.info(f"🎉 Wizard terminé — méthode SMS: {CFG.get('sms_method')}")
...
def audit_auto():
    ...
    while True:
        ...
        if now - dernier_audit >= interval:
            ...
            bilan_jours = CFG.get("bilan_jours", 4)
                        ...
                        try:
                            ...
                            if file_id:
                                ...
                                if texte:
                                    ...
                                    telegram_send("❌ Transcription impossible.")
                                    continue
