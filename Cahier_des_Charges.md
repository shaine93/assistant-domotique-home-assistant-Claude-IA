...
ACCÈS VM — Le deploy server tourne derrière un tunnel Cloudflare.
L'URL change à chaque redémarrage mais est publiée sur ntfy.sh.

ÉTAPE 0 — Récupérer l'URL du tunnel :
python3 -c "
...
lines = resp.read().decode().strip().split(chr(10))
msg = json.loads(lines[-1]) if lines and lines[-1] else {}
print(msg.get('message', 'ERREUR: pas d_url'))
"
→ Donne l'URL HTTPS du deploy server (ex: https://xxx.trycloudflare.com)
→ Utiliser cette URL (appelée BASE ci-dessous) pour toutes les requêtes
...
for line in json.loads(urllib.request.urlopen(req, timeout=10).read().decode())['lines']:
    if 'DEBUG' not in line: print(line[:180])
"

INFRASTRUCTURE :
- VM Google Cloud : lolufe@instance-20260226-111134 / 34.168.105.47
...
- HA : https://philhomeassist.duckdns.org (443→8123, verify=False)
