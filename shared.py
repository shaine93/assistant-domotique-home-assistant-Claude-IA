...
def init_db():
    ...
    # Purge externe supprimée (redondant avec purge inline ci-dessus)
...
def ha_get(endpoint, _retries=2, _delay=5):
    """GET HA API avec retry automatique sur erreurs réseau transitoires."""
    ...
    for attempt in range(_retries + 1):
        try:
            ...
            if attempt < _retries:
                ...
                continue
            # Après tous les retries : WARNING pas ERROR (transitoire réseau)
            log.warning(f"⚠️ HA {endpoint}: réseau indisponible après {_retries+1} tentatives")
        except Exception as e:
