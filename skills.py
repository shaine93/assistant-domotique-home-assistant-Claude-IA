...
# Marqueur de démarrage du service — utilisé par anti-flood heartbeat
_SERVICE_STARTED_AT = datetime.now()
        ...
        if any(k in eid_low for k in ["_etat", "_status"]) and "volume" in eid_low:
            if etat.lower() not in ["normal", "on", "ok", "healthy", ""]:
                try:
                    ...
                    # "attention" = seuil Synology natif (~90%) → ignoré, on utilise notre seuil 95%
                    if etat.lower() == "attention":
                        pass  # Géré par le check pourcentage > 95% ci-dessous
                    else:
    ...
    while True:
        ...
        try:
            ...
            for entity_id, sous_cat, piece in prises:
                ...
                # Hystérésis : détecter si on est en zone standby (10-30W pendant grâce)
                _in_hysteresis = entity_id in _grace_fin and SEUIL_FIN_W < puissance_w < 30
                if puissance_w > SEUIL_FIN_W and not _in_hysteresis:
                    # Machine consomme → cycle en cours ou démarrage
                    ts_now = datetime.now().isoformat()
                    _puissances_historique[entity_id].append((ts_now, puissance_w))
                    # Ne pas reset la grâce si :
                    # 1. Défroissage (rappel envoyé + puissance < 200W)
                    # 2. Standby machine (puissance entre 10W et 30W pendant grace)
                    #    → hystérésis : évite le reset infini sur oscillations standby
                    if _rappel_linge_envoye.get(entity_id) and puissance_w < SEUIL_CYCLE_W:
                        pass  # Défroissage — laisser la grâce expirer normalement
                    elif _in_hysteresis:
                        pass  # Standby — oscillation autour de 10W, ne pas reset la grâce
                    else:
...
def _heartbeat_init_table():
    ...
    Aussi : purge les sensors orphelins (présents dans la table mais plus dans
    _HEARTBEAT_SENSORS_PILIERS), pour éviter les baselines obsolètes après une
    refonte de la liste.
    """
    try:
        conn = sqlite3.connect(DB_PATH)
        ...
        # Purge des sensors orphelins
        try:
            entities_actives = set()
            for entry in _HEARTBEAT_SENSORS_PILIERS:
                if isinstance(entry, tuple):
                    entities_actives.add(entry[0])
                else:
                    entities_actives.add(entry)
            
            rows = conn.execute("SELECT entity_id FROM sensor_heartbeat").fetchall()
            orphelins = [r[0] for r in rows if r[0] not in entities_actives]
            if orphelins:
                placeholders = ",".join("?" * len(orphelins))
                conn.execute(f"DELETE FROM sensor_heartbeat WHERE entity_id IN ({placeholders})", orphelins)
                conn.commit()
                log.info(f"💓 heartbeat: {len(orphelins)} sensor(s) orphelin(s) purgé(s) : {orphelins}")
        except Exception as e:
            log.debug(f"heartbeat purge orphelins: {e}")
...
# Validée 25/04/2026 puis corrigée 05/05/2026 après vérification factuelle des entity_id via /api/states.
#
...
_HEARTBEAT_SENSORS_PILIERS = [
    # Conso temps réel Ecojoko (~30s entre updates en journée)
    ("sensor.ecojoko_consommation_temps_reel",   "none"),
    # Conso réseau Ecojoko
    ("sensor.ecojoko_consommation_reseau",       "none"),
    # Surplus de production = injection solaire → dépend du soleil
    ("sensor.ecojoko_surplus_de_production",     "solar"),
    # APSystems instantané
    ("sensor.ecu_current_power",                 "solar"),
    # APSystems cumulé jour
    ("sensor.ecu_today_energy",                  "solar"),
    # Anker entrée solaire
    ("sensor.solarbank_e1600_puissance_solaire", "solar"),
    # Anker SOC — étouffé si batterie pleine
    ("sensor.solarbank_e1600_etat_de_charge",    "battery_full"),
    # Sentinelle cloud Anker — détecte les pannes API Anker indépendamment de la météo
    ("sensor.solarbank_e1600_etat_du_cloud",     "none"),
]
